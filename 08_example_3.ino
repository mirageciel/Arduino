...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    float led_brightness = (dist_raw - 100) * 127/50 + 1;
    analogWrite(PIN_LED, led_brightness);
  }
  else if(dist_raw >= 200 && dist_raw <= dist_max) {
    float led_brightness = 255 - (dist_raw - 100) * 127/50;
    analogWrite(PIN_LED, led_brightness);
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading >= dist_min && reading <= dist_max) save_reading = reading;
  return save_reading;
  if(reading < dist_min || reading > dist_max) reading = save_reading; // return 0 when out of range.
  return reading;
